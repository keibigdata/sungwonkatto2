...
* 한국환경연구원 텍스트 수집 도구는 2023년 5월 25일까지 네이버 기사 데이터와 환경부 문헌 데이터를 제공한다.아래 뉴스레터에서는 2023년 2월 28일을 기점으로 30일 이전까지 수집한 자료를 사용한다. 
...
1. NAVER 환경 뉴스 : 2023년 1월 30일-2023년 2월 28일 NAVER 뉴스> '사회' >'환경' 부문에 게시된 뉴스
2. 환경부 문건 : 2023년 1월 30일-2023년 2월 28일 환경부홈페이지에 게시된 환경부 보도자료 및 e-환경뉴스
...
### B. 키워드 빈도분석
 ...
 한국환경연구원의 환경 빅데이터 플랫폼에서는 키워드를 단순빈도수 및 TF_IDF 값에 따라 추출한다. 키워드 추출 전단계로 파이선'konlpy'패키지를 활용한 형태소 분석 및 품사 태깅을 수행하고, 불용어(stopwords)를 제거한다.  2023년 1월 30일 - 2월 28일 게시 NAVER 환경뉴스 및 환경부 문건에서 단순빈도수가 높은 20개의 키워드를 추출한 결과는 아래 그림과 같다. 
...
NAVER_TF=read.csv("./Data/2023_02_28/2023-02-28_30_NN_TF.txt",
                  encoding="UTF-8",
                  row.names=NULL,
                  header=FALSE)[,-1]
...
NAVER_TFIDF=read.csv("./Data/2023_02_28/2023-02-28_30_NN_norm_TF.txt",
                  encoding="UTF-8",
                  row.names=NULL,
                  header=FALSE)[,-1]

ME_TF=read.csv("./Data/2023_02_28/2023-02-28_30_ME_TF.txt",
               encoding="UTF-8",
               row.names=NULL,
               header=FALSE)[,-1]

ME_TFIDF=read.csv("./Data/2023_02_28/2023-02-28_30_ME_norm_TF.txt",
               encoding="UTF-8",
               row.names=NULL,
               header=FALSE)[,-1]
...
* 첫째, 네이버 환경뉴스에는 빈도수가 높은 상위 20개 키워드 중 '케이블카','설악산','오색'등 설악산 오색 케이블카 관련 키워드 및 '오염수','원전' 등 원전관련 키워드가 발견되었지만 환경부 문건의 빈도수 상위 20개 키워드에는 이에 상응하는 키워드가 발견되지 않았다.

* 둘째, 환경부 문건의 빈도수 상위 20개 키워드 중에는 '습지','생물' 등 자연환경 관련 키워드, '가뭄','용수' 등 수자원 관련 키워드, '규제','보조금','협약' 등 환경정책 도구 관련 키워드, 그리고 '안전','차량' 등의 키워드가 포함되지만 네이버 환경뉴스에서는 이에 상응하는 키워드가 발견되지 않았다. 
...
* "케이블카","설악산","오색","오염수" 와 같은 키워드는 NAVER 뉴스에서만 찾아볼 수 있고

* "습지","규제","안전","가뭄","생물","국립공원","용수","구제"와 같은 키워드들은 환경부 문건에서만 찾아 볼 수 있다.
...
TITLE_NN_30=read.csv("./Data/2023_02_28/2023-02-28_30_NN_idx.txt",header=FALSE) 
TITLE_ME_30=read.csv("./Data/2023_02_28/2023-02-28_30_ME_idx.txt",header=FALSE) 
...
1. '케이블카'를 제목에 포함하는 기사들은 설악산 오색케이블카의 환경영향 관련 논쟁을 소개한 기사가 대부분이다. 같은 기간 환경부 문헌 제목에 '케이블카','설악산','오색'을 포함하는 문헌은 '설악산국립공원 오색삭도 설치사업 환경영향평가 협의 완료' 1건에 그친다.
...
2. '오염수'을 제목에 포함하는 기사들은 일본 후쿠시마 원전 오염수 방류와 관련된 기사가 주종을 이룬다.  같은 기간 환경부 문건 중 '오염수'를 제목에 포함한 문건은 존재하지 않았다.
...
환경부 문헌에서 가장 빈도가 높은 10개 키워드 중 네이버 뉴스에서 가장 빈도가 높은 키워드 20개에 포함되지 않은 키워드는 "안전", "녹색", "습지","협약","규제","보조금"이다. 2023년 3월 2일부터 3월 31일까지 게시된 환경부 문헌 중 이들 키워드를 제목에 포함하는 환경부 문헌은 다음과 같다. 

1. '안전'을 제목에 포함하는 문헌은 수질관련 문헌 1건, 굴뚝 시료채취 관련 문헌 1건으로 특정한 추세를 보이지 않았다.
...
```
2. '녹색'을 제목에 포함하는 문헌들은 녹색산업 관련 문건이 1건, 녹색금융 관련 문건이 1건이어서 기후변화와 관련된 녹색경제 활성화와 관련된 문건으로 구성되었다.
...
3. '규제'를 제목에 포함하는 문헌들은 환경부문 규제혁신 관련 문헌 2건으로 파악되었다.   
...
4. '습지','보조금'을 제목에 포함하는 문헌들은 각각 습지정책 관련 문헌 2건,   전기차 보조금 관련 문헌이 1건이었다. 
```{r titleME4, echo=FALSE, warning =FALSE}
R2_ME=TITLES %>% filter(SOURCE=="ME") %>%
  filter(GAP==30) %>%
  filter(str_detect(Title,"습지")|str_detect(Title,"보조금")) %>%
  select(Title)
kable(R2_ME)
...
2023년 1월 30일-2023년 2월 28일 게시된 네이버 환경뉴스와 환경부 문헌을 비교한 결과를 요약하면 다음과 같다.

1. 네이버 기사로부터 설악산 오색삭도(오색케이블카) 사업 및 후쿠시마 원전 오염수 방류에 대한 국민들의 관심이 많은데 이와 관련된 환경부 문건의 비중은 낮은 것으로 파악되었다. 반면 설악산 오색삭도 관련 환경부 문헌은 환경영향평가 종료와 같은 정보 전달에 초점을 두고 있다. 이는 후쿠시마 원전 오염수 방료에 대한 국민적인 관심은 많은데 환경부의 정책적 대응은 활발하지 못함을 시사한다. 또한 환경영향평가가 일상화되어서 개별 사안에 대한 국민적인 관심을 현재의 환경영향평가 정책 운용 과정에서 흡수하고 있음을 보여준다.

2. 환경부 문헌으로부터 이 당시 환경부는 녹색경제 활성화, 환경규제 개선 등을 추진하였음을 알 수 있다. 녹색경제 활셩화 정책은 당시 NAVER 뉴스에 출현빈도가 높았던 '기후' 에 대한 관심에 조응하는 정책이라고 할 수 있다.그러나 환경규제 개선 정책은 NAVER 뉴스의 관심을 얻지 못하였다. 
